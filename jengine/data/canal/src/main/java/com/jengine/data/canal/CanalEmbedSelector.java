...
public class CanalEmbedSelector {
  ...
  public CanalEmbedSelector(Long pipelineId, String destination, short clientId,
      String zkClustersString, Long zkClusterId) {
    canalServer = new CanalServerWithEmbedded();
    startServer(pipelineId, destination, clientId, zkClustersString, zkClusterId);
  }

  private void startServer(Long pipelineId, String destination, short clientId,
      String zkClustersString, Long zkClusterId) {
    String filter = "";
  ...
  public Message selector() {
    ...
    while (true) {
      message = canalServer
          .getWithoutAck(clientIdentity, batchSize, batchTimeout, TimeUnit.MILLISECONDS);
