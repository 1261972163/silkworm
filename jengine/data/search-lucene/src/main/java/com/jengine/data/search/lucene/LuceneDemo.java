package com.jengine.data.search.lucene;
...
/**
 * Lucene demo 1、创建内存目录对象Directory； 2、创建索引写入器IndexWriter，该对象既可以把索引写入到磁盘中也可以写入到内存中。
 * 3、利用IndexWriter将指定的数据存入内存目录对象中； <3> 创建IndexSearch 索引查询对象，然后根据关键字封装Query查询对象； <4>
 * 调用search()方法，将查询的结果返回给ScoreDoc ，迭代里面所有的Document对象，显示查询结果； <5> 关闭IndexWriter
 * 写入器，关闭RAMDirectory目录对象。
 *
 * @author nouuid
 * @date 10/25/2016
 * @since 0.1.0
