...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        ...
        # 直接通过调用封装好的函数来计算前向传播的结果。因为验证测试时不关注正则化损失的值
        ...
        # 通过变量重命名的方式来加载模型，这样在前向传播的训练过程中就不需要调用求滑动平均
        # 的函数来获取平均值了。这样训练过程与验证测试二个过程就可以完全共用mnist_inference.py中定义的
        # 前向传播过程。
        ...
        # 变化。这里是个死循环，没考虑主动退出。后续可以考虑通过判断训练过程结束了
        # 则验证测试过程也可以结束了。
        while True:
